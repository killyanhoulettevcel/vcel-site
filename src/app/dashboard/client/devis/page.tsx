...
import { Plus, Pencil, Trash2, RefreshCw, Download, Filter, ArrowRight } from 'lucide-react'
...
const STATUTS = ['en attente', 'accepté', 'refusé', 'expiré']
...
export default function DevisPage() {
  ...
  const docs     = all.filter(f => f.type_facture === 'proforma')
  ...
  const convertirEnFacture = async (doc: Doc) => {
    await fetch('/api/factures', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        ...doc, id: undefined, type_facture: 'facture', statut: 'en attente',
        facture_ref_id: doc.id, numero_facture: '',
  ...
  return (
    <div className="p-4 md:p-8 max-w-6xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="font-display text-2xl font-bold" style={{ color: 'var(--navy)' }}>Devis</h1>
          <p className="text-sm mt-0.5" style={{ color: 'var(--text-muted)' }}>Propositions commerciales</p>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={() => exportCSV(docs, 'devis')} className="btn-ghost text-sm py-2 px-3"><Download size={14} /></button>
          <button onClick={refresh} className="btn-ghost text-sm py-2 px-3"><RefreshCw size={14} className={loading ? 'animate-spin' : ''} /></button>
          <button onClick={() => router.push('/dashboard/client/factures/nouvelle?type=proforma')} className="btn-primary text-sm">
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        <div className="card-glass p-4">
          ...
          <p className="font-display text-xl font-bold" style={{ color: 'var(--navy)' }}>{docs.length}</p>
        </div>
        <div className="card-glass p-4">
          <p className="text-xs font-medium mb-2" style={{ color: 'var(--text-muted)' }}>Acceptés</p>
          <p className="font-display text-xl font-bold" style={{ color: '#22C55E' }}>{docs.filter(d => d.statut === 'accepté').length}</p>
        </div>
        <div className="card-glass p-4">
          <p className="text-xs font-medium mb-2" style={{ color: 'var(--text-muted)' }}>Montant HT total</p>
          <p className="font-display text-xl font-bold" style={{ color: 'var(--navy)' }}>{fmt(docs.reduce((s, d) => s + (d.montant_ht || 0), 0))}</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="card-glass p-12 text-center">
          <p className="font-semibold mb-3" style={{ color: 'var(--text-secondary)' }}>Aucun devis</p>
          <button onClick={() => router.push('/dashboard/client/factures/nouvelle?type=proforma')} className="btn-primary text-sm">+ Nouveau devis</button>
        </div>
      ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b" style={{ borderColor: 'var(--border)' }}>
                  {['N°', 'Client', 'Objet', 'Date', 'Échéance', 'TTC', 'Statut', ''].map(h => (
                ...
                {filtered.map(doc => {
                  ...
                  return (
                    <tr key={doc.id} className="table-row">
                      ...
                      <td className="py-3 px-4 text-xs" style={{ color: 'var(--text-muted)' }}>{doc.date_echeance ? new Date(doc.date_echeance).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short' }) : '—'}</td>
                      <td className="py-3 px-4 text-xs font-semibold" style={{ color: 'var(--text-primary)' }}>{fmt(doc.montant_ttc)}</td>
                      <td className="py-3 px-4">
                      ...
                      <td className="py-3 px-4">
                        <div className="flex items-center gap-1">
                          {doc.statut === 'accepté' && (
                            <button onClick={() => convertirEnFacture(doc)} title="Convertir en facture"
                              className="p-1.5 rounded-lg hover:bg-green-50 transition-colors" style={{ color: '#16A34A' }}>
                              <ArrowRight size={13} />
                            </button>
                          )}
                          <button onClick={() => router.push(`/dashboard/client/factures/${doc.id}`)}
                            className="p-1.5 rounded-lg hover:bg-[var(--bg-secondary)] transition-colors" style={{ color: 'var(--text-muted)' }}>
                            <Pencil size={13} />
                          </button>
                          <button onClick={() => deleteDoc(doc.id)}
                            className="p-1.5 rounded-lg hover:bg-red-50 transition-colors" style={{ color: 'var(--text-muted)' }}>
                            <Trash2 size={13} />
                          </button>
                        </div>
                      </td>
