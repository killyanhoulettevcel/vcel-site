...
// proforma, Devis, Gérez vos devis et propositions commerciales, Aucun devis — créez votre première proposition, BADGE_COLOR, BADGE_BG remplacés
...
export default function DevisPage() {
  ...
  const docs = allDocs.filter(f => (f.type_facture || 'facture') === 'proforma')
  ...
  const STATUTS_DISPO = ['en attente', 'accepté', 'refusé', 'expiré']
    ...
    <div className="p-4 md:p-8 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <div className="flex items-center gap-3 mt-0.5">
            <p className="text-sm" style={{ color: 'var(--text-muted)' }}>Gérez vos devis et propositions commerciales</p>
            {lastUpdate && (
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => exportCSV(docs, 'proforma')} className="btn-ghost text-sm py-2 px-3"><Download size={14} /></button>
          <button onClick={refresh} className="btn-ghost text-sm py-2 px-3"><RefreshCw size={14} className={loading ? 'animate-spin' : ''} /></button>
          <button
            onClick={() => router.push(`/dashboard/client/factures/nouvelle?type=proforma`)}
      ...
      {loading ? (
        ...
        <div className="card-glass p-12 text-center">
          <p className="font-semibold mb-2" style={{ color: 'var(--text-secondary)' }}>Aucun devis — créez votre première proposition</p>
          <button onClick={() => router.push(`/dashboard/client/factures/nouvelle?type=proforma`)}
            className="btn-primary mt-3 text-sm">+ Nouveau devis</button>
        </div>
      ) : (
                        ...
                        <div className="flex items-center gap-1">
                          {doc.statut === 'accepté' && (
                            <button onClick={() => convertirEnFacture(doc)} title="Convertir en facture"
                              className="p-1.5 rounded-lg transition-colors hover:bg-green-50" style={{ color: '#16A34A' }}>
                              <ArrowRight size={13} />
                            </button>
                          )}
