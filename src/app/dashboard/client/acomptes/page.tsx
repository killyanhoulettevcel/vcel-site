...
interface Doc {
  ...
  client_email?: string; client_nom?: string; objet?: string; type_facture?: string; acompte_pct?: number
}
...
const SC: Record<string, { label: string; color: string; bg: string }> = {
  ...
  'payée':      { label: 'Payé',       color: 'text-emerald-600', bg: 'bg-emerald-50 border-emerald-200' },
...
export default function AcomptesPage() {
  ...
  const docs     = all.filter(f => f.type_facture === 'acompte')
    ...
    <div className="p-4 md:p-8 max-w-6xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm mt-0.5" style={{ color: 'var(--text-muted)' }}>Factures d&apos;acompte liées à vos devis</p>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={() => exportCSV(docs, 'acomptes')} className="btn-ghost text-sm py-2 px-3"><Download size={14} /></button>
          <button onClick={refresh} className="btn-ghost text-sm py-2 px-3"><RefreshCw size={14} className={loading ? 'animate-spin' : ''} /></button>
          <button onClick={() => router.push('/dashboard/client/factures/nouvelle?type=acompte')} className="btn-primary text-sm">
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        ...
        <div className="card-glass p-4"><p className="text-xs font-medium mb-2" style={{ color: 'var(--text-muted)' }}>Payés</p><p className="font-display text-xl font-bold" style={{ color: '#22C55E' }}>{docs.filter(d => d.statut === 'payée').length}</p></div>
        <div className="card-glass p-4"><p className="text-xs font-medium mb-2" style={{ color: 'var(--text-muted)' }}>Montant total</p><p className="font-display text-xl font-bold" style={{ color: 'var(--navy)' }}>{fmt(docs.reduce((s, d) => s + (d.montant_ttc || 0), 0))}</p></div>
      </div>
      ...
      {loading ? (
        ...
        <div className="card-glass p-12 text-center">
          <p className="font-semibold mb-3" style={{ color: 'var(--text-secondary)' }}>Aucun acompte</p>
          <button onClick={() => router.push('/dashboard/client/factures/nouvelle?type=acompte')} className="btn-primary text-sm">+ Nouvel acompte</button>
        </div>
      ) : (
                ...
                {filtered.map(doc => {
                  ...
                  return (
                    <tr key={doc.id} className="table-row">
                      ...
                      <td className="py-3 px-4 text-xs font-semibold" style={{ color: 'var(--text-primary)' }}>
                        {fmt(doc.montant_ttc)}
