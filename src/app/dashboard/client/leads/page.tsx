...
const activiteConfig: Record<string, { icon: React.ElementType; color: string }> = {
  creation: { icon: Plus,       color: 'text-cyan-600' },
  statut:   { icon: ArrowRight, color: 'text-purple-600' },
  score:    { icon: Star,       color: 'text-amber-600' },
  note:     { icon: StickyNote, color: 'text-blue-600' },
  email:    { icon: Mail,       color: 'text-orange-600' },
  appel:    { icon: PhoneCall,  color: 'text-emerald-600' },
  relance:  { icon: RefreshCw,  color: 'text-violet-600' },
}
...
// ── Fiche Lead ────────────────────────────────────────────────────────────────
function FicheLead({ lead, onClose, onUpdate, onDelete, onChangeStatut, isMobile }: {
  lead: Lead
  onClose: () => void
  ...
  const col = colonnes.find(c => c.id === currentLead.statut)
  const sc  = scoreConfig[currentLead.score] || scoreConfig['tiède']
  ...
  const handleStatutChange = (statut: string) => {
    setCurrentLead(prev => ({ ...prev, statut: statut as Lead['statut'] }))
    onChangeStatut(lead.id, statut)
  }
    ...
    <>
      ...
      <div className="flex items-start justify-between p-4 md:p-6 border-b border-[var(--border)]">
        <div className="flex items-center gap-3">
          {isMobile && (
            <button onClick={onClose} className="p-2 rounded-xl text-[var(--text-light)] hover:bg-[var(--bg-secondary)] transition-colors -ml-1">
              <ArrowLeft size={18} />
            </button>
          )}
          <div className={`w-10 h-10 md:w-12 md:h-12 rounded-2xl ${sc.bg} border ${sc.border} flex items-center justify-center font-bold text-base md:text-lg ${sc.color}`}>
            {lead.nom.charAt(0).toUpperCase()}
          </div>
          <div>
            <h2 className="font-display font-bold text-[var(--navy)] text-base md:text-lg">{lead.nom}</h2>
            <div className="flex items-center gap-2 mt-0.5 flex-wrap">
              {lead.entreprise && <p className="text-[var(--text-muted)] text-xs">{lead.entreprise}</p>}
              <span className={`text-xs px-2 py-0.5 rounded-full border font-medium ${col?.bg} ${col?.color}`}>{currentLead.statut}</span>
              <span className={`text-xs px-2 py-0.5 rounded-full ${sc.bg} ${sc.color} font-medium`}>{currentLead.score}</span>
            </div>
          </div>
        </div>
        {!isMobile && (
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {/* Changement de statut rapide — visible surtout mobile */}
        <div className="p-4 md:p-6 border-b border-[var(--border)]">
          <p className="text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wide mb-3">🔄 Statut</p>
          <div className="flex gap-2 flex-wrap">
            {colonnes.map(c => (
              <button
                key={c.id}
                onClick={() => handleStatutChange(c.id)}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold border transition-all ${
                  currentLead.statut === c.id
        ...
        {/* Score manuel */}
        <div className="p-4 md:p-6 border-b border-[var(--border)]">
          <p className="text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wide mb-3">🌡️ Score</p>
            ...
            {[
              ...
              <div key={f.label} className="flex items-start gap-2">
                ...
                <div>
                  ...
                  <p className="text-[var(--text-secondary)] text-xs font-medium break-all">{f.val}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="p-4 md:p-6 border-b border-[var(--border)]">
          ...
          <div className="grid grid-cols-2 gap-3 mb-3">
            ...
            <div className="bg-[var(--bg-secondary)] rounded-xl p-3 border border-[var(--border)]">
              <p className="text-[var(--text-light)] text-xs mb-1">Proba. signature</p>
              <div className="flex items-center gap-1">
                <input
          ...
          {editNotes ? (
            <div className="space-y-2">
              ...
              <button
                ...
                disabled={savingField} className="btn-primary text-xs py-2"
              >
            ...
            <div className="flex gap-2 mb-2">
              {(['note', 'appel', 'email'] as const).map(t => (
                <button key={t} onClick={() => setNewType(t)}
                  className={`flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-medium border transition-all capitalize ${
...
export default function LeadsPage() {
  ...
  // Sur mobile, vue liste par défaut
  const [vue, setVue] = useState<'kanban' | 'liste'>('kanban')
  useEffect(() => { if (isMobile) setVue('liste') }, [isMobile])
  ...
  const handleRelance = async () => {
    ...
    try {
      ...
      changeStatut(relanceLead.id, 'contacté')
  ...
  return (
    <div className="p-3 md:p-6 h-full flex flex-col">
      ...
      <div className="flex items-center justify-between gap-3 mb-4 md:mb-6">
        <div>
          <h1 className="font-display text-lg md:text-2xl font-bold text-[var(--navy)]">Leads CRM</h1>
          <div className="flex items-center gap-2 mt-0.5">
            <p className="text-[var(--text-muted)] text-xs md:text-sm hidden sm:block">Gérez et suivez vos prospects</p>
            {lastUpdate && (
        ...
        <div className="flex items-center gap-1.5 md:gap-2">
          {/* Toggle vue — caché sur mobile */}
          <div className="hidden md:flex items-center bg-[var(--bg-secondary)] border border-[var(--border)] rounded-xl p-1">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-3 mb-4 md:mb-6">
        <div className="kpi-card">
          <div className="flex items-center gap-1.5 mb-1.5"><Users size={13} className="text-cyan-600" /><span className="text-[var(--text-muted)] text-xs">Total</span></div>
          <p className="font-display text-2xl md:text-3xl font-bold text-[var(--navy)]">{leads.length}</p>
        </div>
        <div className="kpi-card">
          <div className="flex items-center gap-1.5 mb-1.5"><Flame size={13} className="text-red-500" /><span className="text-[var(--text-muted)] text-xs">Chauds</span></div>
          <p className="font-display text-2xl md:text-3xl font-bold text-[var(--navy)]">{chauds}</p>
        </div>
        <div className="kpi-card">
          <div className="flex items-center gap-1.5 mb-1.5"><Check size={13} className="text-emerald-500" /><span className="text-[var(--text-muted)] text-xs">Conv.</span></div>
          <p className="font-display text-2xl md:text-3xl font-bold text-[var(--navy)]">{tauxConv}<span className="text-[var(--text-muted)] text-base font-normal">%</span></p>
        </div>
        <div className="kpi-card">
          <div className="flex items-center gap-1.5 mb-1.5"><TrendingUp size={13} className="text-violet-600" /><span className="text-[var(--text-muted)] text-xs">CA prévu</span></div>
          <p className="font-display text-xl md:text-2xl font-bold text-[var(--navy)]">{caPrevisionnel.toLocaleString('fr-FR')}€</p>
        </div>
      </div>
          ...
          {colonnes.map(col => {
            ...
            return (
              <div
                ...
                onDragLeave={e => { const rel = e.relatedTarget as Node; if (!e.currentTarget.contains(rel)) setDragOver(null) }}
              ...
              >
                <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--border)]">
                  ...
                  <div className="flex items-center gap-1.5">
                    {colCA > 0 && <span className="text-[10px] text-emerald-600 font-semibold">{colCA.toLocaleString('fr-FR', { maximumFractionDigits: 0 })}€</span>}
      ...
      {(vue === 'liste' || isMobile) && (
        loading ? (
          ...
          <div className="card-glass p-12 text-center">
            ...
            <button onClick={openCreate} className="btn-primary mt-4 text-sm">+ Nouveau lead</button>
          </div>
        ) : (
            ...
            {filtered.map(l => {
              ...
              return (
                <div key={l.id} onClick={() => setFicheLead(l)}
                  className="card-glass p-3 md:p-4 flex items-center justify-between gap-3 hover:border-[var(--border-hover)] transition-all cursor-pointer active:bg-[var(--bg-secondary)]">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-2 text-xs text-[var(--text-muted)]">
                        ...
                        {l.entreprise && <span className="hidden sm:block truncate">· {l.entreprise}</span>}
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    {l.valeur_estimee > 0 && (
                      <span className="text-emerald-600 text-xs font-bold hidden sm:block">{l.valeur_estimee.toLocaleString('fr-FR')}€</span>
                    )}
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium border ${col?.bg} ${col?.color}`}>{l.statut}</span>
      ...
      {showRelanceModal && relanceLead && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
          <div className="bg-white rounded-t-2xl sm:rounded-2xl border border-[var(--border)] shadow-xl w-full sm:max-w-md p-5 sm:p-6">
            ...
            <div className="space-y-3">
              ...
              <textarea value={relanceMessage} onChange={e => setRelanceMessage(e.target.value)}
                placeholder="Message personnalisé (optionnel)..." rows={3}
                className="w-full bg-[var(--bg-secondary)] border border-[var(--border)] rounded-xl px-3 py-2.5 text-[var(--text-primary)] text-sm placeholder:text-[var(--text-light)] focus:outline-none focus:border-cyan-400 resize-none" />
              {relanceErr && (
      ...
      {/* Modal création/édition — bottom sheet sur mobile */}
      {showModal && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
          <div className="bg-white rounded-t-2xl sm:rounded-2xl border border-[var(--border)] shadow-xl w-full sm:max-w-lg p-5 sm:p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-5">
            ...
            <div className="space-y-3">
              {/* Champs en 1 colonne sur mobile, 2 sur desktop */}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="number" value={form.valeur_estimee} onChange={e => setForm({...form, valeur_estimee: e.target.value})}
                    placeholder="0" className="w-full bg-[var(--bg-secondary)] border border-[var(--border)] rounded-xl px-3 py-2.5 text-[var(--text-primary)] text-sm focus:outline-none focus:border-cyan-400" />
                </div>
                <div>
                  <label className="block text-xs text-[var(--text-muted)] mb-1.5 uppercase tracking-wider font-semibold">Proba. (%)</label>
                  <input type="number" min={0} max={100} value={form.probabilite} onChange={e => setForm({...form, probabilite: e.target.value})}
                    placeholder="50" className="w-full bg-[var(--bg-secondary)] border border-[var(--border)] rounded-xl px-3 py-2.5 text-[var(--text-primary)] text-sm focus:outline-none focus:border-cyan-400" />
                </div>
              </div>
              <div>
                  ...
                  {(['chaud', 'tiède', 'froid'] as const).map(s => {
                    ...
                    return (
                      <button key={s} onClick={() => setForm({...form, score: s})}
                        className={`flex-1 py-2.5 rounded-xl text-xs font-semibold border transition-all flex items-center justify-center gap-1.5 ${form.score === s ? styles[s].active : styles[s].inactive}`}>
                        <span>{icons[s]}</span> {s}
                      </button>
                    )
                  })}
